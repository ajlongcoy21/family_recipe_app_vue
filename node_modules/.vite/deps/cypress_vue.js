...
function insertSingleCssFile(cssFilename, document2, el, log) {
  return cy.readFile(cssFilename, { log }).then((css2) => {
    const style = document2.createElement("style");
    style.appendChild(document2.createTextNode(css2));
...
var injectStylesBeforeElement = (options2, document2, el) => {
  if (!el)
    return;
  let stylesheets = [];
  if (typeof options2.stylesheet === "string") {
    stylesheets.push(options2.stylesheet);
  } else if (Array.isArray(options2.stylesheet)) {
    stylesheets = stylesheets.concat(options2.stylesheet);
  }
  if (typeof options2.stylesheets === "string") {
    options2.stylesheets = [options2.stylesheets];
  }
  if (options2.stylesheets) {
    stylesheets = stylesheets.concat(options2.stylesheets);
  }
  insertStylesheets(stylesheets, document2, el);
  let styles = [];
  if (typeof options2.style === "string") {
    styles.push(options2.style);
  } else if (Array.isArray(options2.style)) {
    styles = styles.concat(options2.style);
  }
  if (typeof options2.styles === "string") {
    styles.push(options2.styles);
  } else if (Array.isArray(options2.styles)) {
    styles = styles.concat(options2.styles);
  }
  insertStyles(styles, document2, el);
  let cssFiles = [];
  if (typeof options2.cssFile === "string") {
    cssFiles.push(options2.cssFile);
  } else if (Array.isArray(options2.cssFile)) {
    cssFiles = cssFiles.concat(options2.cssFile);
  }
  if (typeof options2.cssFiles === "string") {
    cssFiles.push(options2.cssFiles);
  } else if (Array.isArray(options2.cssFiles)) {
    cssFiles = cssFiles.concat(options2.cssFiles);
  }
  return insertLocalCssFiles(cssFiles, document2, el, options2.log);
};
...
var hyphenate$1 = cacheStringFunction$2((str) => str.replace(hyphenateRE$1, "-$1").toLowerCase());
var capitalize$1 = cacheStringFunction$2((str) => str.charAt(0).toUpperCase() + str.slice(1));
var toHandlerKey = cacheStringFunction$2((str) => str ? `on${capitalize$1(str)}` : ``);
    ...
    if (mode === 0 || mode === 1) {
      if (!context.inVPre && startsWith(s, context.options.delimiters[0])) {
        ...
        if (s.length === 1) {
          ...
          if (isCompatEnabled$1("COMPILER_NATIVE_TEMPLATE", context) && node && node.tag === "template" && !node.props.some((p) => p.type === 7 && isSpecialTemplateDirective(p.name))) {
...
function parseTag(context, type, parent) {
  ...
  if (isCompatEnabled$1("COMPILER_V_IF_V_FOR_PRECEDENCE", context)) {
    let hasIf = false;
    let hasFor = false;
...
function genVNodeCall(node, context) {
  const { push, helper, pure } = context;
  const { tag, props, children, patchFlag, dynamicProps, directives: directives2, isBlock, disableTracking, isComponent: isComponent2 } = node;
  if (directives2) {
...
function buildProps(node, context, props = node.props, ssr = false) {
  ...
  for (let i = 0; i < props.length; i++) {
    ...
    if (prop.type === 6) {
      ...
      if (name === "is" || isVBind && isStaticArgOf(arg, "is") && (isComponentTag(tag) || isCompatEnabled$1("COMPILER_IS_ON_ELEMENT", context))) {
        continue;
      }
      if (isVOn && ssr) {
        continue;
...
function processSlotOutlet(node, context) {
  ...
  if (nonNameProps.length > 0) {
    const { props, directives: directives2 } = buildProps(node, context, nonNameProps);
    slotProps = props;
...
var transformFilter = (node, context) => {
  if (!isCompatEnabled$1("COMPILER_FILTER", context)) {
    return;
  }
  if (node.type === 5) {
...
function baseCompile(template, options2 = {}) {
  ...
  const ast = isString(template) ? baseParse(template, options2) : template;
  const [nodeTransforms, directiveTransforms] = getBaseTransformPreset();
  transform(ast, extend$2({}, options2, {
    prefixIdentifiers,
    nodeTransforms: [
      ...nodeTransforms,
  ...
  return generate(ast, extend$2({}, options2, {
    prefixIdentifiers
  }));
}
...
function compile(template, options2 = {}) {
  return baseCompile(template, extend$2({}, parserOptions, options2, {
    nodeTransforms: [
...
var stubsMap = /* @__PURE__ */ new WeakMap();
...
var getOriginalVNodeTypeFromStub = function(type) {
  var _a;
  return (_a = stubsMap.get(type)) === null || _a === void 0 ? void 0 : _a.source;
};
var getOriginalStubFromSpecializedStub = function(type) {
  var _a;
  return (_a = stubsMap.get(type)) === null || _a === void 0 ? void 0 : _a.originalStub;
};
var doNotStubComponents = /* @__PURE__ */ new WeakSet();
  ...
  return defineComponent({
    ...
    setup: function(props, _a2) {
      ...
      return function() {
        ...
        return h(tag, propsWithoutSymbols, renderStubDefaultSlot ? slots : void 0);
      };
    }
  });
};
...
function createStubOnceForType(type, factoryFn, cache2) {
  var cachedStub = cache2.get(type);
  if (cachedStub) {
    return cachedStub;
  }
  var stub = factoryFn();
  cache2.set(type, stub);
  return stub;
}
function stubComponents(stubs, shallow, renderStubDefaultSlot) {
  ...
  if (renderStubDefaultSlot === void 0) {
    renderStubDefaultSlot = false;
  }
  var createdStubsMap = /* @__PURE__ */ new WeakMap();
  var createStubOnce = function(type, factoryFn) {
    return createStubOnceForType(type, factoryFn, createdStubsMap);
  };
  transformVNodeArgs(function(args, instance) {
    ...
    var type = nodeType;
    if ((type === Transition || type === BaseTransition) && "transition" in stubs) {
    ...
    if (isComponent(type) || isFunctionalComponent(type)) {
      ...
      if (isComponent(stub)) {
        ...
        var specializedStubComponent_1 = stubFn_1 ? function() {
          var args2 = [];
          for (var _i = 0; _i < arguments.length; _i++) {
            args2[_i] = arguments[_i];
          }
          return stubFn_1.apply(void 0, args2);
        } : __assign({}, unwrappedStub);
        specializedStubComponent_1.props = unwrappedStub.props;
        var specializedStub = createStubOnce(type, function() {
          return specializedStubComponent_1;
        });
      ...
      if (stub === true || shallow) {
        ...
        var newStub = createStubOnce(type, function() {
          return config.plugins.createStubs ? config.plugins.createStubs({
            name: stubName_1,
            component: type
          }) : createStub({
            name: stubName_1,
...
var BaseWrapper = function() {
  ...
  BaseWrapper2.prototype.html = function() {
    return this.getRootNodes().map(function(node) {
...
function createDevTools(events2) {
  return {
...
var VueWrapper = function(_super) {
  ...
  function VueWrapper2(app, vm, setProps) {
    ...
    _this.componentVM = vm && vm.$.devtoolsRawSetupState ? vm.$.proxy : vm;
  ...
  VueWrapper2.prototype.attachNativeEventListener = function() {
    ...
    if (!vm)
      return;
    var emits = vm.$options.emits ? Array.isArray(vm.$options.emits) ? vm.$options.emits : Object.keys(vm.$options.emits) : [];
    ...
    var _loop_1 = function(eventName2) {
      if (emits.includes(eventName2))
        return "continue";
      element.addEventListener(eventName2, function() {
        var args = [];
...
function mount$1(inputComponent, options2) {
  ...
  if (options2 === null || options2 === void 0 ? void 0 : options2.data) {
    ...
    if (isObjectComponent(originalComponent)) {
      ...
      objectComponent.data = function(vm2) {
        return __assign(__assign({}, originalDataFn_1.call(vm2, vm2)), providedData_1);
      };
    } else {
  ...
  var setProps = function(newProps) {
    for (var _i2 = 0, _a2 = Object.entries(newProps); _i2 < _a2.length; _i2++) {
  ...
  stubComponents(global.stubs, options2 === null || options2 === void 0 ? void 0 : options2.shallow, global === null || global === void 0 ? void 0 : global.renderStubDefaultSlot);
...
function mount(componentOptions, options2 = {}) {
  ...
  return cy.then(() => {
    ...
    const el = getContainerEl();
    injectStylesBeforeElement(options2, document2, el);
    ...
    return cy.wrap(wrapper, { log: false }).wait(1, { log: false }).then(() => {
      if (logInstance) {
